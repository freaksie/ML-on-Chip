...
ap_fixed<32, 18, (ap_q_mode) 5, (ap_o_mode)3, 0> layer7_out[1])
{
	...
	else
	{
		...
		sc_bv<64>* input_2_V_tvin_wrapc_buffer = new sc_bv<64>[1];
			...
			// bitslice(31, 0)
			{
				...
				// celement: input_2.V(31, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 0; i_0 += 2)
					{
						...
						{
							sc_lv<32> input_2_V_tmp_mem;
							input_2_V_tmp_mem = (input_2[i_0]).range().to_string(SC_BIN).c_str();
							input_2_V_tvin_wrapc_buffer[hls_map_index].range(31, 0) = input_2_V_tmp_mem.range(31, 0);
                                 	       hls_map_index++;
						}
					}
				}
				...
				// celement: input_2.V(31, 0)
				{
					...
					for (int i_0 = 1; i_0 <= 1; i_0 += 2)
					{
						...
						if (&(input_2[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> input_2_V_tmp_mem;
							input_2_V_tmp_mem = (input_2[i_0]).range().to_string(SC_BIN).c_str();
							input_2_V_tvin_wrapc_buffer[hls_map_index].range(63, 32) = input_2_V_tmp_mem.range(31, 0);
                                 	       hls_map_index++;
						}
					}
				}
